...
All backend code (Cloudflare Workers, Modal endpoints, any server-side code) MUST include structured logging. Every request handler should log:
- Incoming requests (method, path)
- Outgoing responses (status, duration)
- Errors with context (error message, relevant IDs)
- Important state changes (file accepted, job created, status transitions)

Use structured JSON logs via `console.log(JSON.stringify({...}))` in Workers. Include timestamps, log levels, and relevant identifiers (jobId, fileName, etc.) in every log entry. Log warnings for validation failures (bad file type, missing fields). Log errors with stack traces where possible.

# Overall
take notes as you go and put them in scratchpad.md. make sure to look in here as you go to make sure that you are on the right track and u dont make the same mistakes over and over again. make sure to put tracks that you went down and mistakes you made and things you learned there such that we beome better as we go
